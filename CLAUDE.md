...
`Client.ask()` remains an alias, but the MCP tool an agent holds is `whisper`
and only `whisper`. Both sides must read the roster before it works, which is not obvious from the
