...
Each episode you have **one unit of labour**. You choose how to divide it across
...
You then eat what you hold at the end of the episode. Your utility for the
episode is
...
episode holding none of any one good, that term is zero and your utility for the
episode is zero, no matter how much of the other three you have.

At the end of each episode everything you hold is consumed. Nothing carries into
the next episode — not goods, not labour, not unfinished trades. Each episode is
...
**Public to everyone**: the list of goods, the episode number, the schedule, and
...
1. **Production.** One `produce` call per episode, dividing your labour.
...
override, correct or complete any of it. If nobody produces salt this episode,
there is no salt this episode.

## The episode, in order

Each episode runs through four stages, and the system moves between them on a
...
2. **Production** — your one `produce` call for the episode. Messaging stays
...
This round is **{periods} episodes** long. Your abilities and your tastes are
the same in every one of them, and so is everyone else's. You will be told the
episode number in every response.

What carries from one episode to the next is only what you have learned. Goods,
labour and open offers do not. The other traders are the same traders
throughout.
...
- `produce({good: share, ...})` — commit this episode's labour. Shares must be
  at least 0 and sum to at most 1. One call per episode.
