# Is coordination less to reason about? · grounding
...
and commit it **before** the run. `tools/ground.py less-to-reason` prints this bundle.
...
`tools/ground.py less-to-reason --preflight` prints them. A failed gate is a finding and
