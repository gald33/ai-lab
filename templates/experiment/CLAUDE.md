# <the question this experiment asks> · grounding
...
**You are working on this experiment only.** The repo-root `CLAUDE.md` and
...
run. `tools/ground.py <fragment>` prints this bundle and lists existing runs.

Nothing spends until the [`PREFLIGHT.md`](PREFLIGHT.md) gates have a recorded
result on the current commit. `tools/ground.py <fragment> --preflight` prints them.
